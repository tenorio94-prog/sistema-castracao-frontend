...
const convertToProfileData = (vet: Veterinarian, currentRole: Role): ProfileData => {
  return {
    ...
    memberSince: formatDate(vet.createdAt),
    lastAccess: 'Agora',
...
export default function MedicoProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      // Buscar dados do veterinário logado
      try {
        const veterinarian = await VeterinarianService.getMe();
        setProfileData(convertToProfileData(veterinarian, currentUser.role));
      } catch (apiError: any) {
        console.warn('Não foi possível buscar perfil do veterinário:', apiError.message);
        ...
        setProfileData({
          ...
          phone: '',
          cpf: '',
          role: ROLE_LABELS[currentUser.role] || 'Veterinário',
        ...
        toast.warning('Alguns dados do perfil podem estar incompletos.');
      }
    } catch (error: any) {
  ...
  const handleSave = async (data: ProfileData) => {
    ...
    try {
      ...
      const updateData: UpdateVeterinarianData = {
        ...
        email: data.email,
        phone: data.phone.replace(/\D/g, ''), // Remover formatação
        specialty: data.specialty,
      };

      // Atualizar perfil do veterinário
      await VeterinarianService.updateMe(updateData);
  ...
  const handlePasswordChange = async (passData: { current_password: string; new_password: string; confirm_password: string }) => {
    ...
    try {
      // Atualizar senha
      await VeterinarianService.updateMe({ password: passData.new_password });
      ...
      {saving && (
        <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 flex items-center gap-3 shadow-lg">
            ...
            <span className="font-medium text-gray-700">Salvando...</span>
          </div>
        </div>
      )}
    </div>
